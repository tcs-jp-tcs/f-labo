...
export default function BroadcastTable({ weekend }: { weekend: WeekendBroadcast }) {
  ...
  const gridStyle = {
    gridTemplateColumns: `minmax(80px, 1.1fr) minmax(80px, 1fr) repeat(${cols}, 1fr)`,
  } as const;
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-flabo-carbon overflow-hidden">
      ...
      <div className="py-1">
        <div
          className="grid items-center px-3 md:px-5 py-3 border-b border-white/5 font-display tracking-[0.1em] text-[0.6rem] text-flabo-grey uppercase"
          style={gridStyle}
        >
          <span>セッション</span>
          <span>日本時間</span>
        ...
        {weekend.sessions.map((s, idx) => (
          <div
            key={s.session}
            className={`grid items-center px-3 md:px-5 py-2.5 border-b border-white/[0.03] last:border-b-0 text-xs md:text-[0.85rem] ${
              s.session.includes("決勝")
                ? "bg-flabo-red/5 font-bold"
                : idx === weekend.sessions.length - 2
                  ? "bg-white/[0.02]"
                  : ""
            }`}
            style={gridStyle}
