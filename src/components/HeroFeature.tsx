...
export default function HeroFeature({ review }: { review: Review }) {
  const typeMeta = review.raceType ? TYPE_LABEL[review.raceType] : { label: "LATEST REVIEW", cls: "bg-flabo-red text-white" };

  return (
    <div className="hero-glow rounded-2xl border border-white/5 bg-gradient-to-br from-flabo-carbon to-flabo-dark p-7 md:p-9">
      <div className={`inline-flex items-center gap-2 font-display font-bold tracking-[0.24em] text-[0.6rem] uppercase px-3 py-1 rounded mb-4 ${typeMeta.cls}`}>
